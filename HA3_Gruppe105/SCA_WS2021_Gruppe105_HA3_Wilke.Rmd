...
```{r}
wilke = cps_wh_externals[(cps_wh_externals$Year.x >= 2016 | cps_wh_externals$Year.x <= 2020) & cps_wh_externals$region == "Shangh", ]

wilke$Periode = as.numeric(wilke$Periode)

ggplot(data = wilke, aes(x = Periode, y = Baseline)) + 
  geom_point() + 
  geom_smooth(se = FALSE, method = loess)


cps_wh_externals[(cps_wh_externals$Year.x >= 2016 | cps_wh_externals$Year.x <= 2020) & cps_wh_externals$region == "Shangh", ] %>% 
  ggplot(data = ., aes(x = Periode, y = Baseline)) + 
  # Punktwolke
