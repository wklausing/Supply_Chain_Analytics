...
*Fragen:*
*Baseline DataFrame verwenden oder normale cps_wh_externals DataFrame?*
### (1) Modelle erstellen
```{r echo=FALSE}
...
# Anzeigen der Modelle
  ...
  summary(m1_5)
```
### (2) Bewertung der Modelle
(1) Residuenplots 
```{r}
#Residuenplot Modell 1 (Variable AvgHealth)
ggplot(data = NULL, aes(x = m1_1$model$AvgHealth, y = m1_1$model$AvgHealth )) + 
  geom_point()

#Residuenplot Modell 2
ggplot(data = NULL, aes(x = m1_2$model$InternetStability, y = m1_2$model$InternetStability)) + 
  geom_point() + 
  geom_smooth(method = loess)

#Residuenplot Modell 3
ggplot(data = NULL, aes(x = m1_3$model$BusinessConfidence, y = m1_3$model$BusinessConfidence)) + 
  geom_point() + 
  geom_smooth(method = loess)

#Residuenplot Modell 4
ggplot(data = NULL, aes(x = m1_4$model$ParkingSpaceAvailability, y = m1_4$model$ParkingSpaceAvailability)) + 
  geom_point() + 
  geom_smooth(method = loess)

#Residuenplot Modell 5
ggplot(data = NULL, aes(x = m1_5$model$Congestion, y = m1_5$model$Congestion)) + 
  geom_point() + 
  geom_smooth(method = loess)
```
...
evaluation1_1 = rbind(evaluation, data.frame(Model = "m1_1",
                                          MAE = numeric(1),
                                          sMAPE = numeric(1)))
...
evaluation
```
**Auswahl Begruendung zu Aufgabe 10 Nr.3:**
...
```
### (6) Aufbereitung der Variable
```{r}

```
### (7) Modelle erstellen und bewerten
```{r}

```
**Regressionsbewertung zu (7):** 

### (8) Fehlerkennzahlen berechnen
```{r}

```
**Auswahl Begruendung zu (8):**

### (9) Auswahl des geeigneten Modells
```{r}

```
**Regressionsbewertung zu (9):** 
