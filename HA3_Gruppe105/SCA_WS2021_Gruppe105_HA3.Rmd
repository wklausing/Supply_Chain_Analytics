...
model_bewertungsfunktion <- function(evaluation, model) {
          ...
          evaluation = rbind(evaluation, data.frame(Model = 
                               modelname, MAE = numeric(1), sMAPE = numeric(1))) 
...
```
```{r}
# Wir nehmen das beste Modell m1_1 und erstellen ein Residuenplot
ggplot(data = NULL, aes(x = m1_1$model$IFR, y = m1_1$residuals)) + 
  geom_point() + 
  geom_smooth(se = FALSE, method = loess) + 
  ggtitle("Plot m1_1") +
  xlab("IFR") + ylab("Residuals")
```
...
```{r}
# Wir nehmen das beste Modell m1_1_4 und erstellen ein Residuenplot
ggplot(data = NULL, aes(x = m1_1_4$model$IFR, y = m1_1_4$residuals)) + 
  geom_point() + 
  geom_smooth(se = FALSE, method = loess) + 
  ggtitle("Plot m1_1_4") +
  xlab("IFR") + ylab("Residuals")
...
```{r}
# Wir nehmen das beste Modell m1_1_4 und erstellen ein Residuenplot
ggplot(data = NULL, aes(x = m1_1_4_2$model$IFR, y = m1_1_4_2$residuals)) + 
  geom_point() + 
  geom_smooth(se = FALSE, method = loess) + 
  ggtitle("Plot m1_1_4_2") +
  xlab("IFR") + ylab("Residuals")
```
...
```{r}
# Wir nehmen das beste Modell m1_1_4 und erstellen ein Residuenplot
ggplot(data = NULL, aes(x = m1_1_4_2_3$model$IFR, y = m1_1_4_2_3$residuals)) + 
  geom_point() + 
  geom_smooth(se = FALSE, method = loess) + 
  ggtitle("Plot m1_1_4_2_3") +
  xlab("IFR") + ylab("Residuals")
```
...
```{r}
# Wir nehmen das beste Modell m1_1_4 und erstellen ein Residuenplot
ggplot(data = NULL, aes(x = m1_1_4_2_3_5$model$IFR, y = m1_1_4_2_3_5$residuals)) + 
  geom_point() + 
  geom_smooth(se = FALSE, method = loess) + 
  ggtitle("Plot m1_1_4_2_3_5") +
  xlab("IFR") + ylab("Residuals")
```

### Overfitting mit Test-Daten
...
# Kommentar: Das beste Modell und das Test-Modell weisen aehnliche Zahlen auf.
# Ein Overfitting ist hierbei nicht zu finden.
```
...
```{r eval = FALSE}
#TODO
# die anderen Werte wie R^2 bewerten
...
evaluation[evaluation$Model == "Baseline",]$MAE = mean(abs(cps_wh_externals_baseline$IFR - cps_wh_externals_baseline$BaselineIFR))

# sMAPE von Baseline Modell berechnen
evaluation[evaluation$Model == "Baseline",]$sMAPE = smape(cps_wh_externals_baseline$IFR, cps_wh_externals_baseline$BaselineIFR)
...
# Kommentar: Es gab eine Verbesserung im Modell	m1_1_4_2_3 im Vergleich zur Baseline, da MAE und sMAPE kleiner sind als die Fehlerwerte vom Baseline Modell.
```
...
```{r}
#TODO 
#spezifisch zu 202101 ist schwierig. Vorhersage sagt nicht aus ob es nur ein Monat ist.
# Wird das Modell m1_1_4_2_3 genommen oder generell die cps_WH_externals Daten?
...
# predict(model, data)
predictDF = data.frame(predict(predictModel, data=cps_wh_externals))
colnames(predictDF)  = "Vorhersage"

# Runden
predictDF[1] = round(predictDF$Vorhersage, 4)

# Output
head(predictDF)

# Wir nehmen das beste Modell m1_1_4_2_3_5 und erstellen ein Residuenplot
ggplot(data = NULL, aes(x = predictModel$model$IFR, y = predictModel$residuals)) + 
  geom_point() + 
  geom_smooth(se = FALSE, method = loess) + 
  ggtitle("Plot predictModel") +
  xlab("IFR") + ylab("Residuals")

# Kommentar: Wir gehen die Wette ein, da die Vorhersage ueber 0.85 liegt.
# DataFrame cps_wh_externals enthealt bereits alle CPS Warehousing-Dienstleister
# Basierend auf diesen Daten von 2016-2020 wird eine Vorhersage zu 2021 gemacht.
```
...
*Nutzer*: Da unser Regressionsmodell im kommenden Jahr langfristig in die Unternehmensprozesse implementiert wird, stellt sich die Frage, welche Nutzer und welche Prozesse vom Regressionsmodell profitieren koennen und wie sie das Modell verwenden sollen. Hierbei nehmen wir an, dass die
Geschaeftsfuehrung einen Vorrang auf Informationsbereitstellung hat, da diese Stakeholder wichtige und langfristige Entscheidungen im Unternehmen treffen werden. Diese Entscheidungen koennen sie mit Hilfe des Regressionsmodells treffen, da das Modell den Einfluss der Externen Effekte auf die Leistung der Logistikdienstleister uebersichtlich darstellt. Dabei wurden fuenf externe Effekte identifiziert, die vergleichsweise einen deutlichen Einfluss auf die Logistikdienstleister haben.
Weitere Nutzer koennen neben der Geschaeftsfuehrung auch die Abteilungen der Logistik sowie Finanzen sein, da das Modell wichtige Logistik-Kennzahlen wie das Iten Fill Rate (IFR) abbildet.
*Prozesse*: Anhand des Regressionsmodells kann beispielsweise die Logistik-Abteilung Abschaetzungen machen, welche Leistungen der Logistikdienstleister in welchem Zeitraum durchfuehrt und durch welche externen Faktoren beeinflusst wird. Hierbei kann die Logistik-Abteilung die Lieferprozesse sowie SLAs somit besser steuern. Da wir jetzt diese fuenf externen Faktoren wie: Periode, AvgHealth, ParkingSpaceAvailability, InternetStability und Congestion in unserem Modell abgebildet haben.
*Loesung Bereitstellung*:
*Datenbeschaffgung*:
Phase der Datenbeschaffung:
- Datum (Periode kombinieren)
- Merge() von Periode und externen Faktoren
- CPS-WL und Region Japan filtern für Aufgabe 10 relevant
- Normalisierung
- Datentyp ändern (hier nicht berücksichtigt)
. Variablen transformieren (hier nicht berücksichtigt)
- Variablen kombinieren und Modell erstellen


####
Allgemein: 
Korrelation zwischen zwei Variablen ist eine Messung für Assoziationen zwischen IFR und einer weiteren Variable (Externe Faktoren) Item Fill Rate (IFR). Wir untersuchen die Korrelation zwischen IFR und den externen Faktoren hat. Die Korrelation ist keine Messung fuer Kausalitaet. Wenn IFR mit den externen Faktoren korreliert, dann kann es auf verschiedene kausale Beziehungen zurueckfuehren. In unserem Fall kann eine hohe Korrelation entweder durch Zufallentstehen und beschreibt keinen stabilen Zusammenhang, oder die Korrelation ist auf einen kausalen Zusammenhang zurueckzufuehren, aber die Richtung ist nicht eindeutig.

####
Sie haben nun etwas Erfahrung auf dem Gebiet der Distribution von “Limonalytics”. Obwohl Sie stets gewissenhaft fuer die naechsten Monate im Voraus planen, schwankt die Leistung der Logistikdienstleister stark. Daher sollen Sie ein Modell aufstellen, um deren Leistung auf Basis von externen Effekten abzuschaetzen. Sie erhalten daher die externen Effekte der letzten 60 Monate (2016‐2020) sowie die Vorhersage fuer die kommenden 12 Monate (2021). Die Vorhersage der externen Effekte fuer die naechsten Monate ist ‐ selbstverstaendlich ‐ ungenau und ebenso mit statistischen Verfahren erstellt. Dies ist fuer Sie jedoch innerhalb der Analyse nicht relevant. Es sollte jedoch in Ihrer Entscheidung kurz reflektiert werden.
####
