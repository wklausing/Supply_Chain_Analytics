...
transactions[transactions$Month < 10,]$Periode = 
                    paste (transactions[transactions$Month < 10,]$Year, 
                    transactions[transactions$Month < 10,]$Month, sep = "/0")

#Bei Monat größer gleich 10 muss keine 0 hinzugefügt werden.
transactions[transactions$Month >= 10,]$Periode = 
                    paste (transactions[transactions$Month >= 10,]$Year, 
                    transactions[transactions$Month >= 10,]$Month, sep = "/")
...
# Visualisierung der Daten insgesamt 5 Jahre
 ggplot(demand_shangh, aes(x = Periode, y = `Demand in Shangh`, group=1)) + 
  ...
  ylim(12500, 21500)
```
Begruendung zu Aufgabe 4: Wir haben uns fuer einen Liniendiagramm entschieden, 
...
Kommentar Aufgabe 5: In dem visualisierten Liniendiagramm wird ersichtlich, dass 
    ...
    aufeinanderfolgender Periodenabschnitten in der x-Achse in den Monaten
    wiedererkennen. Beispielsweise erkennt man in den Monaten zwischen 
    Januar und Februar, dass die Nachfrage ihren Hochpunkt erreicht und gegen 
    Ende Februar wieder stark abnimmt.
    ...
    verschiedene Annahmen treffen. Wir setzen beispielsweise historische 
    Nachfragewerte als Indikator für die zukuenftige Nachfrage voraus.
    Hierbei duerfen keine signifikanten Veraenderung im Nachfragemuster
    vorliegen. Außerdem es ist ein Liniendiagramm zu nehmen sinnvoll, 
    da wir uns einen guten ersten Uberblick in einem einfachen Modell ueber die 
    Nachfrage verschaffen koennen.
    Um die Ergebnisse der Modellierung fuer unser Produkt sinnvoll zu nutzen, 
    muessen wir annehmen, dass die Informationen quantifizierbar sind. 
    Zudem ist es anzunehmen, dass das Vergangenheitsverhalten andauern wird. 
    Jedoch muessen wir beruecksichtigen, dass Nachfragevorhersagen immer ungenau 
    sind und kurzfristige Vorhersagen genauer sind als Langfristige.
...
*NOCH MAL UEBERPRUEFEN. MFE stimmt hier nicht da zu groß? *
*Kommentar auch bearbeiten*

7) Wie weit weichen durchschnittlich von den Originalwerten ab? 
...
```{r}
#Durchschnittliche Abweichung berechnen (MFE, Mean Forecast Error)
MFE <- mean(as.numeric(ets_shangh$x - ets_shangh$fitted))
MFE
```
Kommentar: Die Modellwerte weichen durchschnittlich 8.817847 von den 
Originalwerten ab. 
Je naeher das MFE bei 0 ist, desto genauer ist die Vorhersage. 
...
Um Trend und Saisonalitaeten in der Zukunft besser zu verstehen, 
muesste man alle Monate analysieren.
...
# konfidenzintervalle rein 
# Der String gibt alle Januar 01 Monate aus
transactions[transactions$Periode %like% "/01" & 
               
               transactions$region=="Peking",]$Sales %>% sum() # Laender noch mitrein

# forecast funktion fehlt
```
Kommentar: ...
