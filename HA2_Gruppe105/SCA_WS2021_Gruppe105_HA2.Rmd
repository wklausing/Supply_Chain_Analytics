...
#Datenbereinigung: 2021/01 entfernen, da es keine Daten enthält.
transactions = transactions[transactions$Periode != "2021/01",]

#Aggregieren Sales auf Region und Periode für Gruppe105 und Lost Sales
demand <- transactions[transactions$Product == "Gruppe105" | transactions$Product == "Lost Sales",] %>% aggregate(Sales~region+Periode, data=., sum) 
...
```{r}
#Examplarische Region ist Südkorea
demand_skorea = c("Periode", "Demand in Skorea") %>% demand[.]
#Examp
ggplot(demand_skorea, aes(x = Periode, y = `Demand in Skorea`)) + 
  geom_bar(stat="identity", fill="steelblue",  width = 1, position='dodge',  linetype = 0) +
  theme(axis.text.x = element_text(angle=90,hjust=1,vjust=1)) + 
  xlab("Periode YYYY/MM")

ggplot(demand_skorea, aes(x = Periode, y = `Demand in Skorea`, group=1)) + 
  geom_line() +
...
```{r}
"Wie sehen Nachfragespikes in 2017/09, 2019/09, und 2020/09. Wieso es in 2018/09 keinen gab sollte noch untersucht werden."
```
...
8) Erstellen Sie eine Nachfragevorhersage fuer ein weiteres Jahr. Visualisieren Sie den Nachfrageverlauf sowie
